...
async function getLearnerStats(userId) {
  ...
  const recentCredentialUpdates =
    credentialStats[0].recentUpdates[0]?.count || 0;
...
async function getRegulatorStats(userId) {
  ...
  const [totalUsers, totalEmployers, totalJobs, newCredentials] =
    await Promise.all([
...
function calculateJobFillRate(jobs) {
  ...
  const filledJobs = jobs.filter(
    (job) => job.status === 'closed' && (job.applicants?.length || 0) > 0,
  ).length;
  ...
  try {
    for (let i = 11; i >= 0; i--) {
      ...
      if (role === 'learner' && data.length > 0) {
        ...
        const cumulativeByMonth = data.filter(
          (c) => new Date(c.createdAt) <= endDate,
        );

        chartData.push({
          name: monthName,
          verified: cumulativeByMonth.filter((c) => c.status === 'verified')
            .length,
          pending: cumulativeByMonth.filter((c) => c.status === 'pending')
            .length,
          rejected: cumulativeByMonth.filter((c) => c.status === 'rejected')
            .length,
        ...
        const activeJobs = monthJobs.filter(
          (j) => j.status === 'active',
        ).length;
        const closedJobs = monthJobs.filter(
          (j) => j.status === 'closed',
        ).length;
