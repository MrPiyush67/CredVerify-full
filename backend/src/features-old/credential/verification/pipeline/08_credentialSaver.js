...
export async function checkDuplicateCertificate(fingerprint) {
  ...
  console.log(
    `🔍 [DUPLICATE-CHECK] Checking for existing certificate with fingerprint: ${fingerprint}`,
  );
  ...
  try {
    ...
    if (existing) {
      ...
      console.log(
        `   Owner: ${existing.user?.name || 'Unknown'} (${existing.user?.email || 'N/A'})`,
      );
    ...
    console.error(
      `❌ [DUPLICATE-CHECK] Error checking for duplicates:`,
      error.message,
    );
...
export async function saveCredential(params) {
  ...
  try {
    ...
    console.log(
      `✅ [CREDENTIAL-SAVER] Credential saved with ID: ${credential._id}`,
    );

    return credential;
  } catch (error) {
    console.error(
      `❌ [CREDENTIAL-SAVER] Failed to save credential:`,
      error.message,
    );
...
export async function uploadAndSaveCredential(params) {
  ...
  const pageUrl =
    verificationData.verificationUrl || verificationData.sourceUrl || '';
  ...
  console.log(
    `✅ [CREDENTIAL-SAVER] IPFS upload successful: ${ipfsResult.cid}`,
  );
  ...
  if (ipfsResult && ipfsResult.cid && fingerprint && pageUrl) {
    ...
    try {
      ...
      console.log(
        `✅ [CREDENTIAL-SAVER] On-chain registration successful: ${chainResult.txHash}`,
      );
    } catch (blockchainError) {
      console.error(
        '❌ [CREDENTIAL-SAVER] Blockchain registration failed:',
      ...
      console.warn(
        '⚠️  [CREDENTIAL-SAVER] Continuing without blockchain registration',
      );
    }
  } else {
    console.warn(
      '⚠️  [CREDENTIAL-SAVER] Missing data for blockchain registration',
    );
  ...
  const uploadedImage = {
    url: ipfsResult
      ? `https://gateway.pinata.cloud/ipfs/${ipfsResult.cid}`
      : '',
    fileName: fileName,
    fileType: 'image/jpeg',
