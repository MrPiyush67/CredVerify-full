...
import {
  extractText,
  filterOcrCandidates,
} from '../pipeline/04_ocrExtractor.js';
import {
  interpretText,
  validateMetadata,
} from '../pipeline/05_llmInterpreter.js';
...
import {
  validateCourseLink,
  detectPlatform,
} from '../pipeline/09_courseLinkValidator.js';
...
export async function verifyFromManualInput(params) {
  ...
  try {
    ...
    console.log(
      `   Screenshot: ${scrapedData.screenshot ? scrapedData.screenshot.length + ' bytes' : 'null (pending implementation)'}`,
    );
    console.log(
      `   Text: ${scrapedData.text ? scrapedData.text.length + ' characters' : 'null (pending implementation)'}\n`,
    );
    ...
    if (imageCandidates.length > 0) {
      console.log(
        ...
        JSON.stringify(imageCandidates[0], null, 2),
      );
    }
    ...
    for (let i = 0; i < imageCandidates.length; i++) {
      ...
      console.log(
        `   Processing candidate ${i + 1}/${imageCandidates.length}: ${candidate.imageUrl}`,
      );
      ...
      try {
        ...
        console.warn(
          `   ⚠️  Failed to process candidate ${i + 1}:`,
          error.message,
        );
      }
    }
    ...
    if (ocrCandidates.length === 0) {
      throw new Error(
        'No images could be processed - all downloads or OCR failed',
      );
    }
    ...
    if (ocrCandidates.length > 1) {
      ...
      console.log(
        `✅ Selected candidate with ${bestCandidate.textLength} characters\n`,
      );
    } else {
      bestCandidate = ocrCandidates[0];
    }
    ...
    for (let i = 0; i < validCandidates.length; i++) {
      ...
      console.log(
        `   Interpreting candidate ${i + 1}/${validCandidates.length}...`,
      );
      ...
      try {
        ...
        console.warn(
          `   ⚠️  LLM failed for candidate ${i + 1}:`,
          error.message,
        );
      }
    }
    ...
    console.log(
      `✅ Successfully interpreted ${interpretedCandidates.length} candidate(s)\n`,
    );
    ...
    if (
      selectedData.learningHours !== null &&
      selectedData.learningHours !== undefined
    ) {
      if (selectedData.learningHours < 7.5 || selectedData.learningHours > 30) {
        console.error(
          `❌ Duration validation failed: ${selectedData.learningHours} hours (must be 7.5-30)\n`,
        );
        throw new Error(
      ...
      console.log(
        `✅ Duration validated: ${selectedData.learningHours} hours (within 7.5-30 range)\n`,
      );
    } else {
      console.warn(
        `⚠️ No learning hours extracted - skipping duration validation\n`,
      );
    }
    ...
    console.log(
      `   Score: ${verification.finalScore}%, Status: ${verification.status}\n`,
    );
    ...
    if (courseUrl) {
      ...
      if (courseLinkValidation.isValid) {
        console.log(
          `✅ Course link is valid: ${courseLinkValidation.domain}\n`,
        );
        ...
        if (scrapeResult.success && scrapeResult.courseData) {
          ...
          if (analysisResult.success) {
            ...
            console.log(
              `📍 STAGE 13: Calculating NCrF credits and NSQF level...`,
            );

            const calculationResult = await calculateNcrfNsqf(
              scrapeResult.courseData,
            );
            ...
            console.log(
              `   NCrF Credits: ${courseAnalysisResult.ncrf?.credits_rounded || 'N/A'}`,
            );
            console.log(
              `   NSQF Level: ${courseAnalysisResult.nsqf?.level || 'N/A'}\n`,
            );
          } else {
            console.warn(
              `⚠️  Course analysis failed: ${analysisResult.error || 'Unknown error'}\n`,
            );
          }
        } else {
          console.warn(
            `⚠️  Course scraping failed: ${scrapeResult.error || 'Unknown error'}\n`,
          );
        }
      } else {
        console.warn(
          `⚠️  Course link invalid: ${courseLinkValidation.reason}\n`,
        );
      }
    } else {
    ...
    if (
      autoSave &&
      finalCandidate.verification.status === 'VERIFIED' &&
      !testMode
    ) {
