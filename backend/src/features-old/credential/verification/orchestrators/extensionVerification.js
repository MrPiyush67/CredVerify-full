...
export async function verifyFromExtension(params) {
  ...
  try {
    ...
    console.log(
      `✅ Certificate image ready (${certificateImage.length} bytes)\n`,
    );
    ...
    if (extractedText && extractedText.length > 50) {
      ...
      console.log(
        `   Using pre-extracted text from extension (${extractedText.length} chars)`,
      );
    ...
    console.log(
      `✅ Extracted recipient: ${extractedData.recipientName || 'Not found'}`,
    );
    ...
    if (
      extractedData.learningHours !== null &&
      extractedData.learningHours !== undefined
    ) {
      if (
        extractedData.learningHours < 7.5 ||
        extractedData.learningHours > 30
      ) {
        console.error(
          `❌ Duration validation failed: ${extractedData.learningHours} hours (must be 7.5-30)\n`,
        );
        throw new Error(
      ...
      console.log(
        `✅ Duration validated: ${extractedData.learningHours} hours (within 7.5-30 range)\n`,
      );
    } else {
      console.warn(
    ...
    console.log(
      `✅ Name match: ${nameValidation.confidence}% - ${nameValidation.reason}\n`,
    );
    ...
    if (courseUrl) {
      ...
      if (courseLinkValidation.isValid) {
        ...
        try {
          ...
          if (scrapeResult.success && scrapeResult.courseData) {
            ...
            if (analysisResult.success) {
              ...
              console.log(
                `📍 STAGE 12: Calculating NCrF credits and NSQF level...`,
              );
        ...
        console.warn(
          `⚠️  Invalid course URL: ${courseLinkValidation.reason}\n`,
        );
      }
    }
    ...
    console.error(
      `\n❌ [EXTENSION-VERIFICATION] Workflow failed:`,
      error.message,
    );
