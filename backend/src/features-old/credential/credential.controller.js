...
export const uploadCredential = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(
      new ApiResponse(201, { credential }, 'Credential uploaded successfully'),
    );
});
...
export const getMyCredentials = asyncHandler(async (req, res) => {
  ...
  const credentials = await credentialService.getMyCredentials(
    req.user._id,
    filters,
  );
  return res
...
export const getCredentialById = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { credential }, 'Credential fetched successfully'),
    );
});
...
export const updateCredential = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { credential }, 'Credential updated successfully'),
    );
});
  ...
  return res
    ...
    .json(
      new ApiResponse(
        201,
        { credential },
        'Credential saved successfully from extension',
      ),
    );
});
...
export const deleteCredential = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, null, 'Credential deleted successfully'));
});
...
export const getVerifiedCredentials = asyncHandler(async (req, res) => {
  const credentials = await credentialService.getVerifiedCredentials(
    req.user._id,
  );
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        { credentials },
        'Public credentials fetched successfully',
      ),
    );
});
...
export const previewCertificate = asyncHandler(async (req, res) => {
  const { credentialName, issueDate, hours, nsqfLevel, recipientName } =
    req.body;

  if (!credentialName || !issueDate || !hours || !nsqfLevel) {
    throw new AppError(
      400,
      'Please provide all credential details for preview',
    );
  }
...
export const issueBulkCredentials = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    throw new AppError(
      400,
      'Invalid input. Credential data and recipients array required.',
    );
  }
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { results }, 'Bulk credential issuance completed'),
    );
});
...
export const getExternalCourses = asyncHandler(async (req, res) => {
  ...
  const result = await externalCoursesService.searchExternalCourses(
    query || '',
    filters,
    pagination,
  );
...
export const getCourseCategories = asyncHandler(async (req, res) => {
  const categories =
    await externalCoursesService.getCourseCategoriesWithCounts();
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          result,
          'Jobs for all sectors fetched successfully',
        ),
      );
  }
});
