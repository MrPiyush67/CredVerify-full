...
import { registerOnChain } from '../../../utils/blockchainService.js';
 ...
 */
export const verifyOrganizationCertificate = async (
  userId,
  certificateData,
  companyName,
  userName,
  ...
  try {
    console.log(
      `[OrgVerification] Starting verification for user ${userId} (${userName}) against ${companyName}`,
    );
    ...
    console.log(
      `[OrgVerification] Image buffer size: ${imageBuffer.length} bytes`,
    );

    // Step 2: Quick initial OCR to extract basic identifiers for database lookup
    console.log(
      '[OrgVerification] Step 2: Quick OCR extraction for database lookup',
    );
    ...
    console.log(
      '[OrgVerification] Step 3: Extracting basic identifiers via LLM',
    );
    ...
    if (!recipientName) {
      throw new Error(
        'Could not extract recipient name from certificate. Please ensure the certificate has a clear recipient name.',
      );
    }
    ...
    if (nameSimilarity < 85) {
      ...
      console.log(
        '[OrgVerification] ✅ Name verification passed:',
        nameSimilarity + '%',
      );
    }
    ...
    console.log(
      '[OrgVerification] Step 4: Checking organization database for existing certificate',
    );
    const matchResult = await matchOrganizationCertificate(
      basicMetadata,
      companyName,
    );
    ...
    if (matchResult.isMatch && matchResult.matchedCertificate) {
      ...
      console.log(
        '[OrgVerification] ✅ Certificate found in database, using stored data',
      );
      ...
      try {
        ...
        console.log(
          '[OrgVerification] Category calculated from DB data:',
          categoryData.category,
        );
      } catch (error) {
        console.warn(
          '[OrgVerification] Category calculation failed:',
          error.message,
        );
        categoryData = {
      ...
      console.log(
        '[OrgVerification] Final metadata from database:',
        finalMetadata,
      );
    } else {
      // No match found or low confidence - use the extracted metadata from LLM
      console.log(
        '[OrgVerification] ⚠️  No database match, using freshly extracted metadata',
      );
      ...
      try {
        ...
        categoryData = await categorizeCourse(courseData);
        console.log(
          '[OrgVerification] Category calculated:',
          categoryData.category,
        );
      } catch (error) {
        console.warn(
    ...
    console.log(
      '[OrgVerification] Step 5.5: Validating microcredential duration',
    );
    ...
    if (learningHours !== null && learningHours !== undefined) {
      if (learningHours < 7.5 || learningHours > 30) {
        console.error(
          `[OrgVerification] ❌ Duration validation failed: ${learningHours} hours`,
        );
        throw new Error(
      ...
      console.log(
        `[OrgVerification] ✅ Duration validation passed: ${learningHours} hours`,
      );
    } else {
      console.warn(
        '[OrgVerification] ⚠️ No learning hours extracted - skipping duration validation',
      );
    }
    ...
    if (matchResult.matchScore >= 85) {
      ...
      isOrganizationVerified = false;
      console.log(
        '[OrgVerification] Low-confidence match: PENDING manual review',
      );
    }
    ...
    if (matchResult.isMatch && matchResult.matchScore >= 60) {
      try {
        ...
        console.log(
          '[OrgVerification] Step 6b: Checking for duplicate certificate',
        );
        ...
        if (existingCredential) {
          console.log(
            `[OrgVerification] ❌ Duplicate certificate detected: ${fingerprint}`,
          );
        ...
        console.log(
          '[OrgVerification] ✅ No duplicate found, proceeding with verification',
        );
        ...
        ipfsResult = await uploadToIpfs(
          imageBuffer,
          certificateData.fileName || 'certificate.jpg',
        );
        console.log(
          `[OrgVerification] IPFS upload successful: ${ipfsResult.cid}`,
        );
        ...
        console.log(
          `[OrgVerification] Blockchain registration successful: ${blockchainTxHash}`,
        );

        // Step 6e: Increment verification count on matched certificate
        if (matchResult.matchedCertificate && autoApproved) {
          const orgCert = await OrganizationCertificate.findById(
            matchResult.matchedCertificate._id,
          );
          if (orgCert) {
            await orgCert.incrementVerificationCount();
            console.log(
              '[OrgVerification] Incremented verification count on matched certificate',
            );
          }
        }
      } catch (error) {
        ...
        console.error(
          '[OrgVerification] Warning: IPFS/Blockchain step failed:',
          error.message,
        );
    ...
    const credentialData = {
      ...
      organizationName:
        matchResult.matchedCertificate?.companyName || companyName,
      ...
      organizationVerification: {
        ...
        matchedCertificateId:
          matchResult.matchedCertificate?._id?.toString() || null,
        matchScore: matchResult.matchScore,
        matchedFields: {
          nameMatch: matchResult.matchDetails?.nameMatch || false,
          certificateIdMatch:
            matchResult.matchDetails?.certificateIdMatch || false,
