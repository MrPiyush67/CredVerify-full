...
} from '../services/organizationVerification.service.js';
...
export const submitCertificate = async (req, res) => {
  try {
    ...
    if (!recipientName || !certificateId || !companyName || !issuer) {
      throw new AppError(
        400,
        'Missing required fields: recipientName, certificateId, companyName, issuer',
      );
    }
    ...
    if (certificateImageBase64) {
      try {
        ...
        const storageBasePath =
          process.env.ORGANIZATION_CERT_STORAGE_PATH ||
        ...
        // Parse base64 image
        const base64Data = certificateImageBase64.replace(
          /^data:image\/\w+;base64,/,
          '',
        );
        ...
        if (
          certificateImageBase64.startsWith('data:image/jpeg') ||
          certificateImageBase64.startsWith('data:image/jpg')
        ) {
          fileExt = 'jpg';
          mimeType = 'image/jpeg';
    ...
    console.log(
      `[Submit Certificate] Created certificate ${certificateId} for ${companyName}`,
    );
    ...
    return res
      ...
      .json(
        new ApiResponse(
          201,
          { certificateId: orgCertificate._id, status: 'pending_processing' },
...
export const getPendingCertificates = async (req, res) => {
  try {
    ...
    const certificates =
      await OrganizationCertificate.getPendingForProcessing(limit);

    console.log(
      `[Get Pending] Returning ${certificates.length} pending certificates`,
    );
...
export const processCertificate = async (req, res) => {
  try {
    ...
    if (processingStatus === 'processed' && ocrText && extractedData) {
      ...
      throw new AppError(
        400,
        'Invalid processing data. Required: processingStatus, ocrText, extractedData',
      );
    }
  } catch (error) {
...
export const verifyUserCertificate = async (req, res) => {
  try {
    ...
    if (!companyName || !certificateImageBase64) {
      throw new AppError(
        400,
        'Missing required fields: companyName, certificateImageBase64',
      );
    }
    ...
    console.log(
      `[Verify Certificate] User ${userId} (${req.user.name}) verifying against ${companyName}`,
    );
