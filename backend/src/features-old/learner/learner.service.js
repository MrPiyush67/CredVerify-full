...
export const getLearnerById = async (
  ...
  if (requestingUserRole === 'employer') {
    const hasAccess = await canEmployerAccessProfile(
      requestingUserId,
      profile._id,
    );
    if (hasAccess) {
...
export const updateSettings = async (userId, updates) => {
  const fieldMappings = {
    ...
    security: ['twoFactorAuth', 'loginNotifications', 'sessionTimeout'],
...
export const getStats = async (userId) => {
  const Credential = (await import('../credential/credential.model.js'))
    .default;
