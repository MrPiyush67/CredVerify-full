  ...
  const credentialStats = await Credential.aggregate([
    {
      $facet: {
        ...
        myVerified: [
          { $match: { verifiedBy: userId, status: 'verified' } },
          { $count: 'count' },
        ],
        myRejected: [
          { $match: { verifiedBy: userId, status: 'rejected' } },
          { $count: 'count' },
        ],
      },
    },
...
export const updateSettings = async (userId, updates) => {
  const fieldMappings = {
    ...
    privacy: ['profileVisibility', 'showEmail', 'showPhone', 'trackActivity'],
    security: [
      'twoFactorAuth',
      'loginNotifications',
      'sessionTimeout',
