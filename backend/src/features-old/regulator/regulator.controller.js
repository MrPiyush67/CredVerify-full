...
export const getPendingCredentials = asyncHandler(async (req, res) => {
  const credentials = await regulatorService.getPendingCredentials(
    req.user._id,
  );
...
export const verifyCredential = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { credential }, 'Credential verified successfully'),
    );
});
...
export const rejectCredential = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { credential }, 'Credential rejected'));
});
...
export const getAllRegulators = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { regulators }, 'Regulators fetched successfully'),
    );
});
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        { user: regulator },
        'Regulator fetched successfully',
      ),
    );
});
...
export const updateSettings = asyncHandler(async (req, res) => {
  const settings = await regulatorService.updateSettings(
    req.user._id,
    req.body,
  );
  return res
