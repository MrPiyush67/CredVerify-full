...
export const requestVerification = async (userId, platform) => {
  ...
  console.log(
    `[Service] Code Match: ${code === profile[platform].verificationCode}`,
  );
  console.log(`[Service] =========================================`);
...
export const verifyPlatformOwnership = async (userId, platform) => {
  ...
  if (!profile[platform].handle) {
    throw new AppError(
      400,
      'Handle not found. Please submit your handle first',
    );
  }
  ...
  if (platform === 'codeforces' && !profile[platform].verificationCode) {
    ...
    console.log(
      `[Service] Stored in DB: "${profile[platform].verificationCode}"`,
    );
  }
  ...
  console.log(
    `[Service] Code from DB: "${profile[platform].verificationCode}"`,
  );
  console.log(
    `[Service] Code Type: ${typeof profile[platform].verificationCode}`,
  );
  console.log(
    `[Service] Code Length: ${profile[platform].verificationCode?.length}`,
  );
  console.log(`[Service] ==========================================`);
  ...
  if (!profile[platform].verificationCode) {
    throw new AppError(
      400,
      'No verification code found. Please request a verification code first',
    );
  }
