...
export const sendMessageDirect = asyncHandler(async (req, res) => {
  ...
  console.log(
    `💬 [CHAT API] User ${req.user._id} sending message to recipient ${recipientId}`,
  );
  ...
  console.log(
    `📋 [CHAT API] Using conversation ${conversation._id} with participants:`,
  ...
  console.log(
    `✅ [CHAT API] Message sent successfully to recipient ${recipientId}`,
  );
