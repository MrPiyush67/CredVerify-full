...
async function testFakeQR() {
  ...
  console.log(`${colors.bright}��� TESTING FAKE QR CODE - No Valid Link${colors.reset}`);
  console.log(`${colors.bright}${'='.repeat(80)}${colors.reset}\n`);

  const fileBuffer = fs.readFileSync(CONFIG.FILE_PATH);
  console.log(`${colors.cyan}��� File: unverified_QR1.jpg (${(fileBuffer.length/1024).toFixed(2)} KB)${colors.reset}`);
  console.log(`${colors.yellow}��� Sending to verification endpoint...${colors.reset}\n`);
  ...
  console.log(`${colors.cyan}⏱️  Processing Time: ${processingTime}ms (${(processingTime/1000).toFixed(2)}s)${colors.reset}\n`);
  ...
  if (!response.ok) {
    ...
    console.log(`${colors.yellow}   ��� Full Response:${colors.reset}`);
    ...
    console.log(`   Auto-Approved: ${verification.autoApproved ? '✓' : '✗'}\n`);

    if (result.data.verificationUrl) {
      console.log(`${colors.yellow}   ��� Extracted URL:${colors.reset}`);
      console.log(`      ${result.data.verificationUrl}\n`);
    }

    if (result.data.extractedData) {
      const data = result.data.extractedData;
      console.log(`${colors.yellow}   ��� Extracted Data:${colors.reset}`);
    ...
    if (verification.breakdown) {
      console.log(`\n${colors.yellow}   ��� Component Scores:${colors.reset}`);
