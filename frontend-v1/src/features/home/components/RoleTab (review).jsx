...
export default function RoleTab({ role, id, tabpanelProps = {} }) {
  ...
  const { items, isLoading } = useMemo(() => {
    ...
    switch (roleKey) {
      case 'learners':
        return {
          items: Array.isArray(users?.data) ? users.data : [],
      ...
      case 'employers':
        return {
          items: Array.isArray(employers?.data) ? employers.data : [],
          isLoading: employers?.loading || false,
        };
      default:
      ...
      switch (role) {
        ...
        case 'employer': {
          ...
          const companySizeMatch =
            !selectedBucket || itemBucket === selectedBucket;
          ...
          <Card className="col-span-full">
            <CardContent className="p-4">
              <p className="text-sm text-muted-foreground">
                {emptyText[role] ?? 'No records found.'}
              </p>
            </CardContent>
          </Card>
        )}
        ...
        {!isLoading &&
          filteredItems.slice(0, displayCount).map((item) => {
            ...
            return (
              <Card
                key={key}
                className="hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 border-border/60 flex flex-col"
              >
                <CardHeader className="pb-3">
