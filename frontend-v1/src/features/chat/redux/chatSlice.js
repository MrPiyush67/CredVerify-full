...
const transformConversation = (conversation, currentUserId) => {
  ...
  if (transformedLastMessage) {
    transformedLastMessage = {
      ...transformedLastMessage,
      isOwn:
        transformedLastMessage.sender?._id?.toString() ===
...
export const fetchConversations = createAsyncThunk(
  ...
  async (_, { rejectWithValue, getState }) => {
    try {
      ...
      const transformed = conversationsList.map((conv) =>
  ...
  reducers: {
    ...
    receiveMessage: (state, action) => {
      ...
      if (message.conversationId === state.activeConversation.id) {
        ...
        const exists = state.messages.data.some(
      ...
      if (conversationIndex !== -1) {
        ...
        state.conversations.data[conversationIndex].lastMessageTime =
          message.createdAt;

        // Move conversation to top
        const conversation = state.conversations.data.splice(
          conversationIndex,
          1,
        )[0];
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(sendMessage.fulfilled, (state, action) => {
        ...
        const conversationIndex = state.conversations.data.findIndex(
          (c) =>
            c._id === message.conversation || c._id === message.conversationId,
        );
        if (conversationIndex !== -1) {
          state.conversations.data[conversationIndex].lastMessage = message;
          state.conversations.data[conversationIndex].lastMessageTime =
      ...
      .addCase(startConversation.fulfilled, (state, action) => {
        ...
        const exists = state.conversations.data.some(
...
export const selectIsTyping = (conversationId) =>
  createSelector(
    [selectRealTime],
