...
export default function AddCredentialsPage() {
  ...
  // Redux state
  const { profile: platformProfile, isLoading: platformLoading } = useSelector(
  ...
  const handleOrganizationVerification = async (verificationResult) => {
    try {
      if (verificationResult.verified || verificationResult.autoApproved) {
        ...
        toast.success(
          'Certificate verified successfully with organization database!',
        );
      } else {
        ...
        toast.warning(
          'Certificate submitted for manual review. No match found in organization database.',
        );
      }
    } catch (error) {
      const errorMessage =
  ...
  const uploadMethods = [
    ...
    {
      id: 'certificate',
      onClick: () => openModal('certificateQr'),
      icon: QrCode,
    ...
    {
      id: 'link',
      onClick: () => openModal('linkVerification'),
      icon: LinkIcon,
  ...
  return (
    <div className="container mx-auto px-4 max-w-7xl">
      ...
      <motion.section
        ...
        className="mb-8"
      >
        <h2 className="text-xl font-semibold mb-4 text-[#116466]">
          Upload Methods
        </h2>
      ...
      {platformHandlers.confirmDelete && (
        <div className="fixed inset-0 z-50">
          <div
            ...
            onClick={platformHandlers.closeDeleteConfirmation}
          />
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-xl w-[90%] max-w-md border">
            <div className="p-6">
              ...
              <div className="mb-4 p-3 rounded-md bg-red-50 text-red-700 text-sm">
                Your {platformHandlers.confirmDelete.name} profile link will be
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button
                  ...
                  onClick={() =>
                    platformHandlers.handleDelete(
                      platformHandlers.confirmDelete.id,
                    )
                  }
                >
                  Confirm
      ...
      {/* Digilocker Modal */}
      <DigilockerModal isOpen={modals.isDigilockerOpen} onClose={closeModal} />
    </div>
  );
}
