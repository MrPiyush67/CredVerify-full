...
const jobsApi = {
  ...
  getApplicantDetails(jobId, applicantUserId) {
    return axiosClient.get(ENDPOINTS.JOBS.APPLICANT_DETAILS(jobId, applicantUserId));
  },
  createJob(jobData) {
  ...
  updateApplicantStatus(jobId, applicantId, status) {
    return axiosClient.patch(ENDPOINTS.JOBS.UPDATE_APPLICANT(jobId, applicantId), { status });
  },
  getJobStats() {
