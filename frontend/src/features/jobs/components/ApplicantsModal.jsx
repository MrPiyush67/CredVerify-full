    ...
    <AnimatePresence>
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex items-start gap-3 flex-1">
                          <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center text-white font-semibold flex-shrink-0">
                            {applicant.credentialist?.name?.charAt(0).toUpperCase() || 'A'}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-2">
                              <h3 className="font-semibold truncate">
                                {applicant.credentialist?.name || 'Unknown'}
                              </h3>
                            ...
                            <div className="flex items-center gap-2 text-sm text-muted-foreground">
                              ...
                              <span className="truncate">{applicant.credentialist?.email || 'N/A'}</span>
                            </div>
                            {applicant.appliedAt && (
      ...
      {selectedApplicant && (
        <ApplicantDetailsModal
          jobId={jobId}
          applicantUserId={selectedApplicant.credentialist?._id}
